-- Approved in STUDENT_LOAN_INTAKE_STORAGE_PLAN.md. Reuses assessments and household RLS.
CREATE OR REPLACE FUNCTION public.validate_student_loan_intake(p_answers jsonb, p_complete boolean)
RETURNS void LANGUAGE plpgsql SET search_path = public, pg_temp AS $$
DECLARE
  c jsonb := public.student_loan_intake_contract();
...
BEGIN
  ...
  FOR s IN SELECT value FROM jsonb_array_elements(c->'sections') LOOP
    ...
    IF p_complete AND active AND s->>'id' IN ('loans','employment','schools') AND jsonb_array_length(rows) = 0 THEN
      RAISE EXCEPTION 'INTAKE:incomplete';
    END IF;
...
-- Definer RPC owns writes. Restrictive policies also block bypasses through the normal REST table API.
CREATE POLICY student_intake_active_reader ON public.assessments AS RESTRICTIVE
  FOR SELECT TO authenticated USING (assessment_type <> 'student_loan_intake' OR public.crm_is_owner() OR public.crm_is_advisor());
CREATE POLICY student_intake_insert_rpc_only ON public.assessments AS RESTRICTIVE
  FOR INSERT TO authenticated WITH CHECK (assessment_type <> 'student_loan_intake');
CREATE POLICY student_intake_update_rpc_only ON public.assessments AS RESTRICTIVE
  FOR UPDATE TO authenticated USING (assessment_type <> 'student_loan_intake') WITH CHECK (assessment_type <> 'student_loan_intake');
CREATE POLICY student_intake_delete_blocked ON public.assessments AS RESTRICTIVE
  FOR DELETE TO authenticated USING (assessment_type <> 'student_loan_intake');

CREATE UNIQUE INDEX assessments_student_intake_active_origin_idx ON public.assessments
  (household_id, (derived_metrics#>>'{intake_origin,kind}'), (derived_metrics#>>'{intake_origin,id}'))
  WHERE assessment_type = 'student_loan_intake' AND status = 'draft' AND deleted_at IS NULL;
CREATE INDEX assessments_student_intake_history_idx ON public.assessments (household_id, created_at DESC)
  WHERE assessment_type = 'student_loan_intake' AND deleted_at IS NULL;

CREATE OR REPLACE FUNCTION public.guard_student_loan_intake()
RETURNS trigger LANGUAGE plpgsql SET search_path = public, pg_temp AS $$
BEGIN
  IF TG_OP <> 'INSERT' AND OLD.assessment_type = 'student_loan_intake' THEN
  ...
  END IF;
  IF TG_OP <> 'DELETE' AND NEW.assessment_type = 'student_loan_intake' THEN
    IF TG_OP = 'UPDATE' AND OLD.assessment_type <> 'student_loan_intake' THEN RAISE EXCEPTION 'INTAKE:invalid_conversion'; END IF;
    IF current_user IN ('authenticated','anon','service_role') OR
      current_setting('crm.student_intake_write',true) IS DISTINCT FROM 'save' THEN RAISE EXCEPTION 'INTAKE:rpc_required'; END IF;
    IF NEW.capture_channel <> 'advisor_onboarding' OR NEW.report_token IS NOT NULL OR NEW.report_path IS NOT NULL
      OR NEW.overall_score IS NOT NULL OR NEW.overall_grade IS NOT NULL OR NEW.deleted_at IS NOT NULL THEN RAISE EXCEPTION 'INTAKE:private_only'; END IF;
    PERFORM public.validate_student_loan_intake(NEW.answers, NEW.status = 'completed');
  END IF;
  IF TG_OP = 'DELETE' THEN RETURN OLD; END IF;
  RETURN NEW;
END;
$$;
REVOKE ALL ON FUNCTION public.guard_student_loan_intake() FROM PUBLIC, anon, authenticated;
CREATE TRIGGER assessments_student_intake_guard BEFORE INSERT OR UPDATE OR DELETE ON public.assessments
  FOR EACH ROW EXECUTE FUNCTION public.guard_student_loan_intake();
...
DECLARE
  row public.assessments%ROWTYPE; source_lead uuid; origin jsonb;
BEGIN
  ...
  PERFORM public.validate_student_loan_intake(p_answers, p_complete);
  ...
  IF p_intake_id IS NULL THEN
    IF p_expected_updated_at IS NOT NULL OR EXISTS (SELECT 1 FROM public.assessments
      WHERE household_id = p_household_id AND assessment_type = 'student_loan_intake' AND status = 'draft'
      AND deleted_at IS NULL AND derived_metrics = origin) THEN RAISE EXCEPTION 'INTAKE:conflict'; END IF;
    PERFORM set_config('crm.student_intake_write','save',true);
    INSERT INTO public.assessments(household_id,lead_id,assessment_type,capture_channel,status,completed_at,answers,derived_metrics)
      VALUES(p_household_id,source_lead,'student_loan_intake','advisor_onboarding',
    ...
    PERFORM public.crm_write_activity(p_household_id,'system','Student Loan intake started',NULL,
      jsonb_build_object('source','student_loan_intake'),NULL,NULL,source_lead,row.id);
  ELSE
    SELECT * INTO row FROM public.assessments WHERE id = p_intake_id AND household_id = p_household_id
      AND assessment_type = 'student_loan_intake' AND capture_channel = 'advisor_onboarding' AND deleted_at IS NULL FOR UPDATE;
  ...
  END IF;
  PERFORM set_config('crm.student_intake_write','',true);
  IF p_complete THEN
    PERFORM public.crm_write_activity(p_household_id,'assessment_completed','Student Loan intake completed',NULL,
      jsonb_build_object('source','student_loan_intake'),NULL,NULL,source_lead,row.id);
  END IF;
  RETURN to_jsonb(row);
END;
