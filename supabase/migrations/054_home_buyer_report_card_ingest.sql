...
-- Enable home_buyer as a first-class public Report Card assessment type.
--
-- Verified gap: public.assessment_type has no home_buyer value, while
...
-- allowlist only family/business/retirement/protection/student_loan/credit.
--
-- Adds enum value home_buyer (018/048/050 ADD VALUE IF NOT EXISTS pattern) and
-- replaces the three existing public ingest/follow-up/duplicate functions
-- so they accept home_buyer. No new ingest RPC. No new tables/columns.
-- Does not create Opportunities. Does not add a mortgage sales vertical.
-- Does not add provider-specific credit schema or credit-pull fields.
-- Does not change Family/Business/Retirement/Protection/Student Loan/Credit
-- lead_type or lead_source mappings.
--
-- home_buyer:
--   lead_type   = Home Buyer Report Card
--   lead_source = home_buyer_report_card

ALTER TYPE public.assessment_type ADD VALUE IF NOT EXISTS 'home_buyer';

COMMENT ON TYPE public.assessment_type IS
  'Assessment product type. family/business/retirement/protection/student_loan/credit/home_buyer are public report-card flows; household_onboarding is CRM guided evidence capture.';
...
BEGIN
  ...
  IF v_review.status IS DISTINCT FROM 'pending' THEN
    ...
    IF v_prior_action IS NOT NULL AND v_prior_action = v_action THEN
      ...
      WHERE lead_id = v_review.incoming_lead_id
        AND assessment_type IN ('family', 'business', 'retirement', 'protection', 'student_loan', 'credit', 'home_buyer')
  ...
  IF v_assessment.capture_channel IS DISTINCT FROM 'public_self_report'
     OR v_assessment.assessment_type NOT IN ('family', 'business', 'retirement', 'protection', 'student_loan', 'credit', 'home_buyer') THEN
    RAISE EXCEPTION 'CRM_DUP:invalid_assessment'
      USING ERRCODE = 'P0001';
  END IF;
