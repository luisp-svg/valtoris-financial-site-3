...
-- Sprint 4A.3 RC: allow confirm_same_household when the only extra provisional
-- dependent is the automatic resolve_possible_duplicate task created by migration 022.
--
-- Minimal change to resolve_public_family_duplicate_review (from 021):
--   1) Exclude that expected resolve task from the unsafe task_count.
--   2) Complete matching open resolve tasks after successful keep_separate / confirm_same.
--
-- Does not redesign 021/022. Does not soften other unsafe-dependent checks.
-- Does not change Financial Progress, canonical contact fields, lead/assessment
-- history, task automation idempotency keys, or activity semantics.
