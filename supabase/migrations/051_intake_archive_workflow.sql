...
-- Safe Intake archive primitive. No new columns or tables.
--
-- Authenticated UPDATE of leads.deleted_at fails SELECT-on-NEW because
-- leads_select requires deleted_at IS NULL. Browser Activity INSERT is
-- revoked. The existing browser activity RPC does not allow Intake archive events.
-- This SECURITY DEFINER RPC performs the three writes in one transaction:
--   1) set leads.deleted_at
--   2) complete only a verified ordinary linked follow-up task
--   3) write one private CRM Activity
--
-- Does not complete resolve_possible_duplicate or
-- resolve_digital_identity_duplicate. Pending duplicate review blocks archive.
-- Unassigned-pool visibility is not mutation permission.
-- Rejects 'Manual Contact' and any other non-allowlisted lead_type.
...
COMMENT ON FUNCTION public.archive_intake_lead(uuid, text) IS
  'Authenticated Intake archive. Sets leads.deleted_at only. Owner or assigned-household advisor. Rejects pending duplicate review. Completes only the linked ordinary follow-up task (review_initial_diagnostic | review_digital_identity_lead). Never completes resolve_possible_duplicate or resolve_digital_identity_duplicate. Writes one private CRM Activity via crm_write_activity. Reasons: dismissed | not_a_fit | spam | test_or_accidental.';
