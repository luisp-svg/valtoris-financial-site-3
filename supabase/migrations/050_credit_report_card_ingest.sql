...
-- Enable credit as a first-class public Report Card assessment type.
--
-- Verified gap: public.assessment_type has no credit value, while
...
-- allowlist only family/business/retirement/protection/student_loan.
--
-- Adds enum value credit (018/048 ADD VALUE IF NOT EXISTS pattern) and
-- replaces the three existing public ingest/follow-up/duplicate functions
-- so they accept credit. No new ingest RPC. No new tables/columns.
-- Does not create Opportunities. Does not enable the disabled servicing module.
-- Does not change Family/Business/Retirement/Protection/Student Loan
-- lead_type or lead_source mappings.
--
-- credit:
--   lead_type   = Credit Report Card
--   lead_source = credit_report_card

ALTER TYPE public.assessment_type ADD VALUE IF NOT EXISTS 'credit';

COMMENT ON TYPE public.assessment_type IS
  'Assessment product type. family/business/retirement/protection/student_loan/credit are public report-card flows; household_onboarding is CRM guided evidence capture.';
...
BEGIN
  ...
  -- Idempotent replay (any of the six public types for this UUID).
  ...
  LEFT JOIN public.assessments a
   ...
   AND a.assessment_type IN ('family', 'business', 'retirement', 'protection', 'student_loan', 'credit')
...
COMMENT ON FUNCTION public.ingest_public_report_card(jsonb) IS
  'Atomic public Report Card CRM ingest for family/business/retirement/protection/student_loan/credit. Callable only with service_role. Validates allowlisted assessment_type ↔ lead_type pairs. Creates household/member when needed, always creates new lead+assessment history, never overwrites trusted matches or invents arbitrary types.';
...
BEGIN
  ...
  IF v_review.status IS DISTINCT FROM 'pending' THEN
    ...
    IF v_prior_action IS NOT NULL AND v_prior_action = v_action THEN
      ...
      WHERE lead_id = v_review.incoming_lead_id
        AND assessment_type IN ('family', 'business', 'retirement', 'protection', 'student_loan', 'credit')
  ...
  IF v_assessment.capture_channel IS DISTINCT FROM 'public_self_report'
     OR v_assessment.assessment_type NOT IN ('family', 'business', 'retirement', 'protection', 'student_loan', 'credit') THEN
    RAISE EXCEPTION 'CRM_DUP:invalid_assessment'
      USING ERRCODE = 'P0001';
  END IF;
