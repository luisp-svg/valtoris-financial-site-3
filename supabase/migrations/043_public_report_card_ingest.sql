...
-- Generalize public Report Card CRM ingest (approved parameterization).
--
-- Adds ingest_public_report_card(jsonb) for the four public assessment types:
--   family / Family Report Card
--   business / Business Report Card
--   retirement / Retirement Report Card
--   protection / Protection Gap
--
-- ingest_public_family_report_card remains a compatibility wrapper that forces
-- family + Family Report Card (existing db tests and callers keep working).
--
-- Also extends create_public_family_follow_up_task and
-- resolve_public_family_duplicate_review so Intake review + follow-up tasks
-- work for all four public_self_report types. Does not create tables, does not
-- change scoring formulas, does not create Opportunities.
...
BEGIN
  ...
  -- Idempotent replay (any of the four public types for this UUID).
  ...
  LEFT JOIN public.assessments a
   ...
   AND a.assessment_type IN ('family', 'business', 'retirement', 'protection')
...
COMMENT ON FUNCTION public.ingest_public_report_card(jsonb) IS
  'Atomic public Report Card CRM ingest for family/business/retirement/protection. Callable only with service_role. Validates allowlisted assessment_type ↔ lead_type pairs. Creates household/member when needed, always creates new lead+assessment history, never overwrites trusted matches or invents arbitrary types.';
...
-- Family compatibility wrapper: force family + Family Report Card.
...
BEGIN
  ...
  RETURN public.ingest_public_report_card(
...
COMMENT ON FUNCTION public.ingest_public_family_report_card(jsonb) IS
  'Compatibility wrapper for ingest_public_report_card. Forces assessment_type=family and lead_type=Family Report Card. service_role only.';
...
-- ---------------------------------------------------------------------------
-- Follow-up task RPC: allow the four public_self_report assessment types
-- ---------------------------------------------------------------------------
...
BEGIN
  ...
  IF NOT FOUND
     ...
     OR v_assessment.assessment_type NOT IN ('family', 'business', 'retirement', 'protection')
...
BEGIN
  ...
  IF v_review.status IS DISTINCT FROM 'pending' THEN
    ...
    IF v_prior_action IS NOT NULL AND v_prior_action = v_action THEN
      ...
      WHERE lead_id = v_review.incoming_lead_id
        AND assessment_type IN ('family', 'business', 'retirement', 'protection')
  ...
  IF v_assessment.capture_channel IS DISTINCT FROM 'public_self_report'
     OR v_assessment.assessment_type NOT IN ('family', 'business', 'retirement', 'protection') THEN
    RAISE EXCEPTION 'CRM_DUP:invalid_assessment'
      USING ERRCODE = 'P0001';
  END IF;
