...
export default function InsuranceQuoteDetail({ quote }: { quote: NonNullable<IntakeQueueItem['insuranceQuote']> }) {
  return <section className="crm-intake-detail-section"><h3>{QUOTE_LABELS[quote.kind]}</h3><p className="crm-muted">Quote requested · Self-reported information for advisor review. No coverage is bound. AgentCRM handoff is not enabled.</p><p>Preferred contact: {quote.preferredContact ?? 'Not provided'}</p>
    {activeSections(quote.kind, quote.answers).map(section => <details key={section.id}><summary>{section.title}</summary><dl className="crm-intake-kv">{activeFields(section.fields, quote.answers).map(field => {
