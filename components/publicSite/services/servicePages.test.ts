...
describe('Phase 3 Student Loan and Credit service pages', () => {
  ...
  it('renders complete Student Loan EN and ES pages with Report Card and Book CTAs', () => {
    ...
    expect(en).toContain('Student Loan Services and Pricing')
    expect(en).toContain('Income-Driven Repayment (IDR)')
    expect(en).toContain('$299')
    expect(en).toContain('$1,200')
    expect(en).toContain('Starting at $799')
    expect(en).toContain('Special Cases')
    expect(en).not.toContain('$299.99')
    expect(en).toContain('Start with the Report Card')
    expect(en).not.toContain('Buy Now')
    expect(en).not.toContain('Checkout')
    ...
    expect(es).toContain('Servicios y precios de préstamos estudiantiles')
    expect(es).toContain('Pago basado en los ingresos (IDR)')
    expect(es).toContain('Desde $799')
    expect(es).toContain('Empezar con el Reporte')
  ...
  it('renders complete Credit EN and ES pages without bureau-score claims', () => {
    ...
    expect(en).toContain('Choose the Level of Support That Fits You')
    expect(en).toContain('DIY Credit Builder')
    expect(en).toContain('$39')
    expect(en).toContain('Basic Credit Support')
    expect(en).toContain('$99')
    expect(en).toContain('Premium Credit Restoration')
    expect(en).toContain('$149')
    expect(en).toContain('No payment is collected through the Credit Report Card')
    expect(en).toContain('billed only after the applicable work is performed')
    expect(en).toContain('Book My Credit Consultation')
    ...
    expect(es).toContain('Elija el nivel de apoyo adecuado para usted')
    expect(es).toContain('$39')
    expect(es).toContain('$99')
    expect(es).toContain('$149')
    expect(es).toContain('Agendar mi consulta de crédito')
