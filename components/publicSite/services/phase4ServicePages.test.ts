...
describe('Phase 4 insurance, health, and business formation service pages', () => {
  ...
  it('has complete EN/ES catalogs for all three pages', () => {
    ...
    expect(insuranceServiceCopy.en.heroTitle).toBe('Protect Your Home. Your Business. Your Future.')
    expect(insuranceServiceCopy.es.heroTitle).toBe('Proteja su hogar. Su negocio. Su futuro.')
  ...
  it('renders valid primary and secondary CTAs for each page', () => {
    ...
    expect(insurance).toContain('Request a Coverage Review')
    expect(insurance).toContain(`href="${ROUTES.schedule}"`)
    expect(insurance).toContain('Explore All Solutions')
    expect(insurance).toContain(`href="${ROUTES.solutions}"`)
    expect(insurance).toContain('Clearview Insurance Agency')
    expect(insurance).toContain('/images/property-business-insurance-hero.webp')
  ...
  it('preserves locale and attribution through the three service journeys', () => {
    ...
    expect(insurance).toContain(`href="${ROUTES.solutions}?${suffix}"`)
    ...
    expect(insurance).toContain('Proteja su hogar. Su negocio. Su futuro.')
