...
import { localizeFamilyScoreResult } from './localizeResults'
...
describe('Family Report Card Spanish localization', () => {
  it('gives Spanish every English copy key', () => {
    ...
    for (const [section, keys] of Object.entries(catalogKeys(familyCopy.en!))) {
  ...
  it('keeps canonical answer values identical between locales', () => {
    ...
    expect(answers.financial.monthlyCashFlow).toBe('break-even')
    expect(answers.goals.selected).toEqual(['protect-family', 'debt-free'])
    expect(t('en', 'answers', 'maritalStatus.married')).toBe('Married')
    expect(t('es', 'answers', 'maritalStatus.married')).toMatch(/Casad/)
  })

  it('renders landing, assessment, and results in Spanish without changing routes', () => {
    const landing = renderAt('/report-card?lang=es', createElement(FamilyReportCardPage))
    expect(landing).toContain('¿Qué tan preparada está su familia en lo financiero?')
    expect(landing).not.toContain('How Financially Prepared Is Your Family?')
    const assessment = renderAt('/family-assessment?lang=es', createElement(FinancialProtectionAssessment))
    expect(assessment).toContain('Comience su Family Financial Report Card™')
    expect(assessment).toContain('Obtenga gratis mi Family Financial Report Card™')
    const results = renderAt('/family-results?lang=es', createElement(FamilyReportCardResults))
    expect(results).toContain('Family Financial Report Card™')
    expect(source('pages/FamilyReportCardPage.tsx')).not.toContain('/es/')
  })

  it('preserves assessment state across locale switch', () => {
    expect(source('pages/FinancialProtectionAssessment.tsx')).toContain('useState<DemoAssessmentAnswers>')
    expect(source('components/assessment/specialized/SpecializedLocaleSwitcher.tsx')).toContain('replace: true')
    expect(
      withSpecializedLocale('/family-assessment', 'es', '?utm_source=qa&utm_campaign=spanish&card=test-card'),
    ).toBe('/family-assessment?utm_source=qa&utm_campaign=spanish&card=test-card&lang=es')
    expect(
      withSpecializedLocale('/family-assessment', 'en', '?lang=es&utm_source=qa&utm_campaign=spanish&card=test-card'),
    ).toBe('/family-assessment?utm_source=qa&utm_campaign=spanish&card=test-card')
    const switcher = renderAt(
      '/family-assessment?lang=es',
  ...
  it('produces identical scores and canonical priorities in both locales', () => {
    ...
    const english = localizeFamilyScoreResult(scored, answers, (section, key) => t('en', section, key))
    const spanish = localizeFamilyScoreResult(scored, answers, (section, key) => t('es', section, key))
    ...
    expect(english.protectionGapAmount).toBe(spanish.protectionGapAmount)
    expect(english.categories.map((item) => `${item.id}:${item.score}:${item.grade}:${item.status}`)).toEqual(
      spanish.categories.map((item) => `${item.id}:${item.score}:${item.grade}:${item.status}`),
    )
    expect(english.priorities.map((item) => item.level)).toEqual(spanish.priorities.map((item) => item.level))
    expect(english.priorities[0]?.title).not.toBe(spanish.priorities[0]?.title)
    expect(source('components/assessment/scoring/scoreFamilyAssessment.ts')).not.toContain('familyCopy')
    expect(source('components/assessment/scoring/scoreFamilyAssessment.ts')).not.toContain("from '../family/copy'")
  })
  ...
  it('does not change ingest, scoring files, credit_repair, or add Migration 053', () => {
    ...
    expect(source('server/ingest/familyReportCard/ingestFamilyReportCard.ts')).not.toContain('familyCopy')
    expect(source('components/reportCard/familyIngest/buildFamilyIngestPayload.ts')).not.toContain('lang=es')
  })
})
