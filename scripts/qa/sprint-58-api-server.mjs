...
const PORT = Number(process.env.QA58_API_PORT || 5180)
const VITE = process.env.QA58_VITE_ORIGIN || 'http://127.0.0.1:5174'
const ROOT = process.cwd()

async function bundleHandlers() {
  const outdir = mkdtempSync(join(ROOT, '.tmp-qa58-api-'))
...
async function main() {
  ...
  const server = http.createServer(async (req, res) => {
    try {
      ...
      console.error('qa58 api error', err?.message || err)
      if (!res.headersSent) {
        res.statusCode = 500
  ...
  server.listen(PORT, '127.0.0.1', () => {
    console.log(`QA58 API+proxy on http://127.0.0.1:${PORT} → ${VITE}`)
  })
}
