...
BEGIN
 ...
 RESET ROLE;
 FOREACH service IN ARRAY ARRAY['credit_repair','retirement','wills_trusts','pc','health','tax_strategy','business','home_buyer'] LOOP
  contract:=public.additional_service_intake_contract(service);sections:='{}';
  FOR sec IN SELECT value FROM jsonb_array_elements(contract->'sections') LOOP
  ...
  END LOOP;
  intake:=jsonb_build_object('version',1,'tracks',jsonb_build_array(service),'sections',sections);
  SET LOCAL ROLE authenticated;
  draft:=public.save_service_intake(h,'member',m,NULL,NULL,intake,false);
  PERFORM pg_temp.expect_rejection(format('SELECT public.save_service_intake(%L,''member'',%L,%L,%L,%L::jsonb,true)',h,m,draft->>'id',draft->>'updated_at',intake),'INTAKE:incomplete');
  PERFORM pg_temp.expect_rejection(format('SELECT public.save_service_intake(%L,''member'',%L,NULL,NULL,%L::jsonb,false)',otherh,m,intake),'INTAKE:unavailable');
  RESET ROLE;
  FOR sec IN SELECT value FROM jsonb_array_elements(contract->'sections') WHERE NOT coalesce((value->>'repeatable')::boolean,false) LOOP
   ...
   FOR field IN SELECT value FROM jsonb_array_elements(sec->'fields') WHERE value->>'required'='true' LOOP
    fields:=fields||jsonb_build_object(field->>'id',CASE field->>'kind' WHEN 'select' THEN field->'options'->>0 WHEN 'number' THEN '2026' WHEN 'date' THEN '2026-01-01' WHEN 'email' THEN 'synthetic@example.invalid' ELSE 'Synthetic' END);
   END LOOP;
   intake:=jsonb_set(intake,ARRAY['sections',sec->>'id','0'],fields);
  END LOOP;
  SET LOCAL ROLE authenticated;
  saved:=public.save_service_intake(h,'member',m,(draft->>'id')::uuid,(draft->>'updated_at')::timestamptz,intake,true);
  IF saved->>'status'<>'completed' THEN RAISE EXCEPTION 'Service completion failed'; END IF;
  PERFORM pg_temp.expect_rejection(format('SELECT public.save_service_intake(%L,''member'',%L,%L,%L,%L::jsonb,false)',h,m,saved->>'id',saved->>'updated_at',intake),'INTAKE:preserved_record');
  RESET ROLE;
 END LOOP;
 SET LOCAL ROLE authenticated;
 IF (SELECT count(*) FROM public.client_intake_progress(h,'member',m))<>8 THEN RAISE EXCEPTION 'Progress missing services'; END IF;
...
END $$;
SELECT 'all additional services lifecycle and security checks passed' AS verification;
ROLLBACK;
