...
BEGIN
 ...
 VALUES(household,'Synthetic Student Intake QA',pipeline,stage,advisor_id),(other_household,'Synthetic Other Household',pipeline,stage,NULL);
 ...
 FOR s IN SELECT value FROM jsonb_array_elements(public.student_loan_intake_contract()->'sections') LOOP
 ...
 complete_a := jsonb_set(a,'{tracks}','["General review"]');
 FOR s IN SELECT value FROM jsonb_array_elements(public.student_loan_intake_contract()->'sections') WHERE value->>'id' IN ('client','loans') LOOP
   SELECT jsonb_object_agg(value->>'id',''::text) INTO r FROM jsonb_array_elements(s->'fields');
   FOR f IN SELECT value FROM jsonb_array_elements(s->'fields') WHERE value->>'required'='true' LOOP
     r := r || jsonb_build_object(f->>'id',CASE f->>'kind' WHEN 'select' THEN f->'options'->>0 WHEN 'email' THEN 'synthetic@example.invalid' ELSE 'Synthetic QA' END);
   END LOOP;
 ...
 PERFORM public.validate_student_loan_intake(a,false);
 PERFORM public.validate_student_loan_intake(complete_a,true);
 PERFORM pg_temp.expect_rejection(format('SELECT public.validate_student_loan_intake(%L::jsonb,true)',a),'INTAKE:incomplete');
 PERFORM pg_temp.expect_rejection(format('SELECT public.validate_student_loan_intake(%L::jsonb,false)',jsonb_set(a,'{sections,client,0,ssn}','"not-allowed"')),'INTAKE:invalid_fields');
 PERFORM pg_temp.expect_rejection(format('SELECT public.validate_student_loan_intake(%L::jsonb,false)',jsonb_set(complete_a,'{sections,loans,0,disbursedAt}','"2026-02-30"')),'INTAKE:invalid_date');
 PERFORM pg_temp.expect_rejection(format('SELECT public.validate_student_loan_intake(%L::jsonb,false)',jsonb_set(complete_a,'{sections,loans,0,balance}','"-1"')),'INTAKE:invalid_amount');
 PERFORM pg_temp.expect_rejection(format('SELECT public.validate_student_loan_intake(%L::jsonb,true)',jsonb_set(complete_a,'{tracks}','["PSLF review"]')),'INTAKE:incomplete');
 PERFORM pg_temp.expect_rejection(format('SELECT public.validate_student_loan_intake(%L::jsonb,false)',jsonb_set(a,'{tracks}','["General review","General review"]')),'INTAKE:invalid_structure');
 IF has_function_privilege('anon','public.save_student_loan_intake(uuid,text,uuid,uuid,timestamptz,jsonb,boolean)','EXECUTE') THEN RAISE EXCEPTION 'Anonymous save allowed'; END IF;
 IF has_function_privilege('authenticated','public.validate_student_loan_intake(jsonb,boolean)','EXECUTE') THEN RAISE EXCEPTION 'Internal validator exposed'; END IF;
 cmd := format('SELECT public.save_student_loan_intake(%L,''contact'',%L,NULL,NULL,%L::jsonb,false)',household,contact,a);
 ...
 saved := public.save_student_loan_intake(household,'contact',contact,NULL,NULL,a,false);
 ...
 PERFORM pg_temp.expect_rejection(format('SELECT public.save_student_loan_intake(%L,''contact'',%L,NULL,NULL,%L::jsonb,false)',other_household,contact,a),'INTAKE:unavailable');
 PERFORM pg_temp.expect_rejection(format('SELECT public.save_student_loan_intake(%L,''report_card'',%L,%L,%L,%L::jsonb,false)',household,report,saved->>'id',saved->>'updated_at',a),'INTAKE:unavailable');
 PERFORM pg_temp.expect_rejection(format('SELECT public.save_student_loan_intake(%L,''contact'',%L,%L,''2000-01-01'',%L::jsonb,false)',household,contact,saved->>'id',a),'INTAKE:conflict');
 ...
 PERFORM pg_temp.expect_rejection(format('INSERT INTO public.assessments(household_id,assessment_type,capture_channel,status,completed_at,answers) VALUES(%L,''student_loan_intake'',''advisor_onboarding'',''draft'',NULL,%L::jsonb)',household,a),'INTAKE:rpc_required');
 updated := public.save_student_loan_intake(household,'contact',contact,(saved->>'id')::uuid,(saved->>'updated_at')::timestamptz,complete_a,false);
 IF updated->>'updated_at' = saved->>'updated_at' THEN RAISE EXCEPTION 'Revision did not advance'; END IF;
 PERFORM pg_temp.expect_rejection(format('SELECT public.save_student_loan_intake(%L,''contact'',%L,%L,%L,%L::jsonb,false)',household,contact,saved->>'id',saved->>'updated_at',a),'INTAKE:conflict');
 completed := public.save_student_loan_intake(household,'contact',contact,(updated->>'id')::uuid,(updated->>'updated_at')::timestamptz,complete_a,true);
 IF completed->>'status' <> 'completed' OR completed->>'completed_at' IS NULL THEN RAISE EXCEPTION 'Completion failed'; END IF;
 PERFORM pg_temp.expect_rejection(format('SELECT public.save_student_loan_intake(%L,''contact'',%L,%L,%L,%L::jsonb,false)',household,contact,completed->>'id',completed->>'updated_at',a),'INTAKE:preserved_record');
 PERFORM pg_temp.expect_rejection(format('SELECT public.save_student_loan_intake(%L,''contact'',%L,NULL,NULL,%L::jsonb,true)',household,contact,complete_a),'INTAKE:save_draft_first');
 new_review := public.save_student_loan_intake(household,'contact',contact,NULL,NULL,a,false);
 IF new_review->>'id' = completed->>'id' THEN RAISE EXCEPTION 'History replaced'; END IF;
 saved := public.save_student_loan_intake(household,'report_card',report,NULL,NULL,a,false);
